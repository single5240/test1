...
#define LOG_TAG "modbus1_task"
#define LOG_OUTPUT_LEVEL  LOG_INFO
#include "log.h"


modbus_rtu_t mbr_no2;
...
static void fsm_reconfig_speed_mode_callback(void *parm)
{
		mbr_dev_mode_set(&mbr_no2,MBR_DEV_SPEED_MODE);
...
static void mbr_no2_unpack_ok_callback(void* this_p, uint8_t* buf, uint16_t len)
{
...
static void mbr_no2_init(uint8_t dev_type,uint8_t dev_addr ,uint8_t mbr_delay)
{
		uint8_t name_buf[] = "mbr_no2";
		mbr_no2.obj_name = name_buf;
		mbr_no2.obj_name_len = sizeof(name_buf);
	
		mbr_no2.usart_send_f = usart2_transmit;
		mbr_up_ok_callback_register(&mbr_no2,mbr_no2_unpack_ok_callback);
	
		mbr_no2.dev_type = dev_type;																			//modbus设备，华远、伟创具体设备
		mbr_no2.dev_addr = dev_addr;
		mbr_no2.dev_mbr_delay = mbr_delay;
	
		if(mbr_no2.dev_type == MBR_DEV_SD710)															//设备支持函数注册
		{
				mbr_no2.unpack_03h_callback_f = sd710_03h_unpack;									//modbus 03h 协议解包函数
				mbr_no2.dev_pos_cfg_f = sd710_pos_mode_config;
				mbr_no2.dev_pos_set_f = sd710_pos_set;
				mbr_no2.dev_speed_cfg_f = sd710_speed_mode_config;
				mbr_no2.dev_speed_set_f = sd710_speed_set;
				mbr_no2.dev_read_test_f = sd710_read_test;
				mbr_no2.dev_read_pos_f = sd710_read_pos;
				mbr_no2.dev_output_enable_f = sd710_output_enable;
				mbr_no2.dev_torque_cfg_f = NULL;
				mbr_no2.dev_torque_set_f = NULL;
			
				mbr_no2.dev_mode = MBR_DEV_POS_MODE;
		}
		else if(mbr_no2.dev_type == MBR_DEV_HS100)
		{
				mbr_no2.unpack_03h_callback_f = hs100_03h_unpack;									//modbus 03h 协议解包函数
				mbr_no2.dev_pos_cfg_f = hs100_pos_mode_config;
				mbr_no2.dev_pos_set_f = hs100_pos_set;
				mbr_no2.dev_speed_cfg_f = hs100_speed_mode_config;
				mbr_no2.dev_speed_set_f = hs100_speed_set;
				mbr_no2.dev_read_test_f = hs100_read_test;
				mbr_no2.dev_read_pos_f = hs100_read_pos;
				mbr_no2.dev_output_enable_f = hs100_output_enable;
				mbr_no2.dev_torque_cfg_f = hs100_torque_mode_config;
				mbr_no2.dev_torque_set_f = hs100_torque_set;
			
				mbr_no2.dev_mode = MBR_DEV_SPEED_MODE;
		}
			
	
		led_obj_create(&mbr_no2.led_L,RJ45_LED_L2_GPIO_Port,RJ45_LED_L2_Pin,1);
		led_obj_create(&mbr_no2.led_R,RJ45_LED_R2_GPIO_Port,RJ45_LED_R2_Pin,1);
		mbr_no2.led_L.set_led_f(&mbr_no2.led_L,0);
		mbr_no2.led_R.set_led_f(&mbr_no2.led_R,0);
	
		mbr_fsm_config(&mbr_no2);
}


void modbus_usart2_rx_callback(uint8_t *buff, uint16_t len)
{
		MBR_AnalyzeApp(&mbr_no2,buff,len);
		
		mbr_no2.led_R.set_led_f(&mbr_no2.led_R,1);
}
...
{	
		...
		mbr_no2_init(MBR_DEV_SD710,HS100_ADDRESS_1,5);
		mbr_no2.fsm_init_f(&mbr_no2,fsm_table,fsm_table_lenth,STATE_DISCONNECT);
    ...
    uint32_t period = osKernelSysTick();
        ...
        EventMsgGetLast(&nolistSubs, RC_DATA_MSG, &modbus_rc_data, NULL);
				...
				mbr_led_process(&mbr_no2);
			
				fsm_stateHandle(&mbr_no2);
